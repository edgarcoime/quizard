...
@router.put("/")
def create(answer: AnswerCreateRequest, db=Depends(get_db), user=Depends(verify_user_exit)):
...
def update(answer_id: str, answer: AnswerUpdateRequest, db=Depends(get_db), user=Depends(verify_user_exit)):
...
def delete(answer_id: str, db=Depends(get_db), user=Depends(verify_user_exit)):
    db_answer = get_answer(db, answer_id)
    if db_answer and db_answer.card and db_answer.card.collection and db_answer.card.collection.user and db_answer.card.collection.user.id == user.id:
